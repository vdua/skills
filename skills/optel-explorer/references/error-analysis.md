...
This workflow enables AI agents to analyze JavaScript errors from RUM (Real User Monitoring) data, identify duplicate errors across different browsers and deployments, and generate comprehensive analysis reports. It uses advanced similarity algorithms optimized for cross-browser error comparison.

## Input

The workflow receives a **file path** pointing to a JSON file containing error data from RUM queries (e.g., `output/errors-jan26-2026.json`). The file contains error facet values from the `optel-query.jsh` script executed with `--facet-values error`.
...
- **`improved-error-similarity.jsh`** - Command-line utility for error analysis and duplicate detection
  ...
  - Can be executed directly:
    ...
    # Node
    node skills/optel-explorer/scripts/improved-error-similarity.jsh <input-file> <output-prefix>
    ```
...
Input File → Run Utility → Generate Analysis Reports
    ...
    node skills/optel-explorer/scripts/improved-error-similarity.jsh <input-file> <output-prefix>   # Node
                ↓
...
This workflow receives a **file path** (e.g., `output/errors-jan26-2026.json`) containing error data from RUM queries.
...
# Node
node skills/optel-explorer/scripts/improved-error-similarity.jsh <input-file> <output-prefix> [--threshold 0.6]
```
...
# Node
node skills/optel-explorer/scripts/improved-error-similarity.jsh output/my-run/errors.json errors-analysis
node skills/optel-explorer/scripts/improved-error-similarity.jsh output/my-run/errors.json errors-analysis --threshold 0.7
```
...
1. ✅ **Execute** `improved-error-similarity` (SLICC) or `node skills/optel-explorer/scripts/improved-error-similarity.jsh` (Node) — detect with `command -v improved-error-similarity`
